...
function setup() {
  ...
  // Establish the range of the Mandelbrot set
  const w = 4;
  ...
  for (let x = 0; x < width; x++) {
    for (let y = 0; y < height; y++) {
      ...
      // Map pixel position to a point in the Mandelbrot set
      let a = map(x, 0, width, xMin, xMin + w);
      let b = map(y, 0, height, yMin, yMin + h);
      const ca = a;
      const cb = b;
      ...
      // Iterate the Mandelbrot calculation
      while (n < maxIterations) {
        const aa = a * a - b * b;
        const bb = 2 * a * b;
        a = aa + ca;
        b = bb + cb;
        if (abs(aa + bb) > 16) {
          break;  // Bail out if it's diverging
        }
        n++;
      }
      ...
      let bright = map(n, 0, maxIterations, 0, 255);
      if (n === maxIterations) bright = 0; // Make it black if it's in the set
